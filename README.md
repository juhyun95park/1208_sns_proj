...
### 🗄️ 데이터베이스 통합
- **Supabase 공식 방식**: Cookie-based 인증 지원 (`@supabase/ssr`)
- **Clerk 통합**: Third-party auth provider 방식 (JWT 템플릿 불필요)
- 환경별 Supabase 클라이언트 분리:
  - **Supabase 공식**: `createClient()` (server/client)
  - **Clerk 통합**: `useClerkSupabaseClient()` / `createClerkSupabaseClient()`
  - **관리자 권한**: `getServiceRoleClient()`
- SQL 마이그레이션 시스템
...
2. **Integrations** 메뉴로 이동하거나, 직접 [Supabase integration setup](https://dashboard.clerk.com/setup/supabase) 페이지로 이동
3. Supabase 통합을 활성화:
   - 통합을 활성화하면 **Clerk domain**이 표시됩니다 (예: `your-app.clerk.accounts.dev`)
   - 이 **Clerk domain**을 복사해 둡니다 (다음 단계에서 사용)

> **참고**: 이 통합은 Clerk의 세션 토큰에 `"role": "authenticated"` 클레임을 자동으로 추가합니다.

**3-2. Supabase Dashboard에서 Clerk 인증 제공자 설정**

1. [Supabase Dashboard](https://supabase.com/dashboard)로 돌아가기
2. 프로젝트 선택 → **Authentication** → **Sign In / Up** → **Third Party Auth**
3. **"Add provider"** 클릭하고 **"Clerk"** 선택
4. Clerk Dashboard에서 복사한 **Clerk domain**을 입력
5. **"Save"** 클릭
...
- [프로젝트 통합 가이드](./docs/clerk-supabase-integration.md)에서 상세한 설정 방법 확인
...
> **참고**: Supabase 공식 문서에서는 `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`를 사용하지만, 기존 코드와의 호환성을 위해 `NEXT_PUBLIC_SUPABASE_ANON_KEY`도 지원합니다.
...
│   │   ├── server.ts          # Server Component용 (Supabase 공식 방식)
│   │   ├── client.ts          # Client Component용 (Supabase 공식 방식)
│   │   ├── clerk-client.ts    # Client Component용 (Clerk 통합)
│   │   ├── clerk-server.ts    # Server Component용 (Clerk 통합)
